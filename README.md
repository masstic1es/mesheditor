...
- 🎯 Entry points to execute your program using `python -m <mesheditor>` or `$ mesheditor` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/masstic1es/mesheditor/branch/main/graph/badge.svg?token=mesheditor_token_here)](https://codecov.io/gh/masstic1es/mesheditor)
[![CI](https://github.com/masstic1es/mesheditor/actions/workflows/main.yml/badge.svg)](https://github.com/masstic1es/mesheditor/actions/workflows/main.yml)

Awesome mesheditor created by masstic1es

## Install

from source
```bash
git clone https://github.com/masstic1es/mesheditor mesheditor
cd mesheditor
make install
```
...
```bash
$ mesheditor run --port 8080
```

or

```bash
python -m mesheditor run --port 8080
```

or

```bash
$ uvicorn mesheditor:app
```
...
```bash
❯ mesheditor --help
Usage: mesheditor [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ mesheditor create-user --help
Usage: mesheditor create-user [OPTIONS] USERNAME PASSWORD
...
```bash
mesheditor create-user admin admin --superuser
```
...
Run with `mesheditor run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `mesheditor create-user`.
...
-----------------------------------------------------
mesheditor/__init__.py              4      0   100%
mesheditor/app.py                  16      1    94%
mesheditor/cli.py                  21      0   100%
mesheditor/config.py                5      0   100%
mesheditor/db.py                   10      0   100%
mesheditor/models/__init__.py       0      0   100%
mesheditor/models/content.py       47      1    98%
mesheditor/routes/__init__.py      11      0   100%
mesheditor/routes/content.py       52     25    52%
mesheditor/routes/security.py      15      1    93%
mesheditor/routes/user.py          52     26    50%
mesheditor/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from mesheditor.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in mesheditor/default.toml.

### As environment variables
```bash
export mesheditor_KEY=value
export mesheditor_KEY="@int 42"
export mesheditor_KEY="@jinja {{ this.db.uri }}"
export mesheditor_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
mesheditor_ENV=production mesheditor run
```
